export default defineBackground(() => {
      ...
      if (result.selectorConfigs && Array.isArray(result.selectorConfigs)) {
        ...
        configurations = [
          ...
          },
          {
            ...
            hamburgerButtonSelector:
              "button.inline-flex.items-center.justify-center.p-1.rounded-md",
          },
        ];
  ...
  async function submitPlayerData() {
    ...
    try {
      ...
      if (currentUsername === "Not found" || !currentUsername) {
        ...
        if (
          ...
          if (clicked) {
            ...
            await fetchBalance();
            console.log("After hamburger click:", {
              username: currentUsername,
              balance: currentBalance,
            });
      ...
      // Submit to API
      console.log(
        "Sending POST request to https://fun-analyzer.kravanh.dev/api/players",
      );
      const response = await fetch(
        "https://fun-analyzer.kravanh.dev/api/players",
        {
          method: "POST",
          headers: {
            "key": "dataforme",
  ...
  function connectPusher() {
    const wsUrl =
      "wss://ws-fun-analyzer.kravanh.dev/app/9cpbe4mbozz6fjriingr?protocol=7&client=js&version=8.4.0&flash=false";
      ...
      pusherWs.onmessage = async (event) => {
        ...
        try {
          ...
          if (data.event === "pusher:error") {
            const errorData = typeof data.data === "string"
          ...
          if (data.event === "pusher:connection_established") {
            ...
            const confidence = match?.prediction_confidence ||
            ...
            addWsMessage(
              `🔮 Fight #${fightNum}: ${predictedResult} (${confidence}%)`,
            );
            broadcastToPopup({
            ...
            if (limitBalance) {
              ...
              if (
                !isNaN(currentBalanceNum) && !isNaN(limit) &&
                currentBalanceNum >= limit
              ) {
                autoBettingEnabled = false;
                await browser.storage.local.set({ autoBettingEnabled: false });
                addWsMessage(
                  `🛑 Limit reached! Balance: ${currentBalance} >= Limit: $${limit}`,
                );
                broadcastToPopup({
                  type: "autoBettingDisabled",
                ...
                if (
                  ...
                  addWsMessage(
                    `🔄 Username not found, clicking hamburger to refresh...`,
                  );
                  const clicked = await clickHamburgerButton();

                  if (clicked) {
                    addWsMessage(
                      `✅ Hamburger button clicked, waiting for UI update...`,
                    );
                    await sleep(1500); // Wait for UI to update
            ...
            data.event === "match.ended" || data.event === ".match.ended"
          ) {
            ...
            addWsMessage(
              `📋 Channel: ${matchData?.match?.channel_id} (Selected: ${selectedChannel})`,
            );
            ...
            console.log(
              "⏰ Setting 5-second timeout for player data submission and page refresh",
            );
            addWsMessage(`⏰ Waiting 5 seconds before submission...`);
            setTimeout(async () => {
              try {
                console.log(
                  "⏰ Timeout triggered, starting player data submission...",
                );
            ...
            data.event === "player.updated" || data.event === ".player.updated"
          ) {
            ...
            if (currentUsername === "Not found" || !currentUsername) {
              console.log(
                "⚠️ Username not found, attempting to fetch before sync check...",
              );
              console.log("Current configuration:", selectedConfigName);
              console.log(
                "Username selector:",
                currentSelectors.usernameSelector,
              );
              addWsMessage(
                `🔄 Fetching username for sync check (config: ${selectedConfigName})...`,
              );

              // For Option 2, click hamburger button FIRST before fetching
              if (selectedConfigName === "Option 2") {
                console.log(
                  "Option 2 detected - clicking hamburger button FIRST...",
                );
                console.log(
                  "Hamburger selector:",
                  currentSelectors.hamburgerButtonSelector,
                );
                ...
                if (clicked) {
                  console.log(
                    "Hamburger button clicked, waiting 1.5s for menu to open...",
                  );
                  await sleep(1500); // Wait for UI to update
              ...
              if (
                (currentUsername === "Not found" || !currentUsername) &&
                selectedConfigName === "Option 1"
              ) {
                console.log(
                  "⚠️ Username still not found with Option 1 selector",
                );
              ...
              if (currentUsername === "Not found" || !currentUsername) {
                ...
                addWsMessage(
                  `   Using selector: ${currentSelectors.usernameSelector}`,
                );
              } else {
                console.log(
            ...
            if (shouldSync) {
              ...
              if (currentUsername === "Not found" || !currentUsername) {
                console.log(
                  "📝 Using username from player.updated event:",
                  player.username,
                );
                addWsMessage(
                  `📝 Setting username from server: ${player.username}`,
                );
              ...
              console.log(
                "🔄 Username matches! Syncing settings from server...",
              );
              addWsMessage(
                `🔄 Syncing settings from server for ${player.username}...`,
              );
              ...
              const newBetAmount = player.bet_amount?.toString() ||
                oldBetAmount;
              if (oldBetAmount !== newBetAmount) {
                currentSelectors.inputValue = newBetAmount;
                changes.push(`Bet Amount: ${oldBetAmount} → ${newBetAmount}`);
                console.log(
                  `✏️ Updated bet_amount: ${oldBetAmount} → ${newBetAmount}`,
                );
              }

              // Sync limit_balance
              const oldLimitBalance = limitBalance;
              const newLimitBalance = player.limit_balance?.toString() ||
                oldLimitBalance;
              if (oldLimitBalance !== newLimitBalance) {
                limitBalance = newLimitBalance;
                changes.push(
                  `Limit Balance: ${oldLimitBalance} → ${newLimitBalance}`,
                );
                console.log(
                  `✏️ Updated limit_balance: ${oldLimitBalance} → ${newLimitBalance}`,
                );
              }
              ...
              if (oldChannel !== newChannel) {
                ...
                console.log(
                  `✏️ Updated channel_id: ${oldChannel} → ${newChannel}`,
                );
              }
              ...
              if (oldAutoBet !== newAutoBet) {
                ...
                console.log(
                  `✏️ Updated is_auto_bet: ${oldAutoBet} → ${newAutoBet}`,
                );
              }
              ...
              if (oldManualBet !== newManualBet) {
                ...
                console.log(
                  `✏️ Updated is_manual_bet: ${oldManualBet} → ${newManualBet}`,
                );
              }
              ...
              if (changes.length > 0) {
                addWsMessage(
                  `✅ Synced ${changes.length} setting(s) from server`,
                );
              ...
              console.log(
                "⏭️ Username does not match current user, skipping sync",
              );
              addWsMessage(
                `👤 Player update for ${player.username} (not current user)`,
              );
            }
            ...
            addWsMessage(
              `   Username: ${player.username}, Balance: $${player.balance}`,
            );
            ...
            data.event === "bet.placed" || data.event === ".bet.placed"
          ) {
            ...
            addWsMessage(`🎰 ${message}`);
            addWsMessage(
              `   ${betType}: $${totalBetAmount} (${validBettersCount} betters)`,
            );
            ...
            if (currentUsername === "Not found" || !currentUsername) {
              console.log(
                "⚠️ Username not found for bet.placed, attempting to fetch...",
              );
              console.log("Current configuration:", selectedConfigName);
            ...
            if (players && Array.isArray(players)) {
              ...
              currentUserInBet = players.find((p: any) =>
              ...
              if (
                ...
                console.log(
                  "⚠️ Username not found, checking first player in array",
                );
                currentUserInBet = players[0];
                console.log(
                  "📝 Using player from bet event:",
                  currentUserInBet.username,
                );
                addWsMessage(
                  `📝 Setting username from bet event: ${currentUserInBet.username}`,
                );
            ...
            if (currentUserInBet) {
              console.log(
                "✅ Current user found in bet.placed players:",
                currentUserInBet,
              );
              addWsMessage(
                `🔄 Processing bet.placed event for ${currentUserInBet.username}...`,
              );

              // Follow the bet from server (always bet when event received)
              console.log("📍 bet.placed event received - will place bet");
              addWsMessage(`📍 Following bet from server...`);
              ...
              if (limitBalance) {
                ...
                if (
                  ...
                  setTimeout(async () => {
                    console.log(
                      "⏰ 5 seconds elapsed after bet placement, submitting player data...",
                    );
              ...
              const oldBetAmount = currentSelectors.inputValue;
              const newBetAmount = currentUserInBet.bet_amount?.toString() ||
                oldBetAmount;
              if (oldBetAmount !== newBetAmount) {
                currentSelectors.inputValue = newBetAmount;
                changes.push(`Bet Amount: ${oldBetAmount} → ${newBetAmount}`);
                console.log(
                  `✏️ Updated bet_amount from bet: ${oldBetAmount} → ${newBetAmount}`,
                );
              }
              ...
              if (currentUserInBet.is_manual_bet !== undefined) {
                ...
                if (oldManualBet !== newManualBet) {
                  ...
                  changes.push(`Manual-Bet: ${oldManualBet} → ${newManualBet}`);
                  console.log(
                    `✏️ Updated is_manual_bet from bet: ${oldManualBet} → ${newManualBet}`,
                  );
                }
              }
              ...
              if (changes.length > 0) {
                ...
                // Log summary of changes
                addWsMessage(
                  `✅ Synced ${changes.length} setting(s) from bet.placed`,
                );
                changes.forEach((change) => {
                  addWsMessage(`   ${change}`);
                  console.log(`   ${change}`);
              ...
              console.log(
                "⏭️ Current user not found in bet.placed players array",
              );
  ...
  browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.action === "testBet") {
      ...
      addWsMessage(
        `🧪 Test bet triggered: ${betType === "red" ? "MERON" : "WALA"}`,
      );
  ...
  browser.runtime.onInstalled.addListener(async () => {
    ...
    try {
      const existingScripts = await browser.scripting
