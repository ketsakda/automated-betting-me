  ...
  main() {
    ...
    browser.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      ...
      if (message.action === 'automate') {
        ...
        const checkboxSelector = message.checkboxSelector || 'input[type="checkbox"]';
      ...
      if (message.action === 'getUsername') {
        ...
        const allTextRightPs = document.querySelectorAll('p[class*="text-right"]');
        ...
        let usernameElement: Element | null = document.querySelector('p.text-sm.text-right.pr-1.leading-none');
        if (usernameElement) {
        ...
        if (!usernameElement) {
          const parentDiv = document.querySelector('div.text-white.fill-white.text-right');
          if (parentDiv) {
        ...
        console.log('Sending response:', { username, debugCount: debug.length });
